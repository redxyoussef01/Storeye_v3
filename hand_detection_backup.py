...
cropped_videos_dir = "./data/cropped_videos"

# Get all video files in the cropped_videos directory
video_files = [f for f in os.listdir(cropped_videos_dir) if f.endswith('.mp4')]
video_files.sort()  # Process videos in alphabetical order
...
for video_idx, video_filename in enumerate(video_files):
    print(f"\n{'='*60}")
    print(f"Processing video {video_idx + 1}/{len(video_files)}: {video_filename}")
    print(f"{'='*60}")
    
    # Construct full video path
    video_path = os.path.join(cropped_videos_dir, video_filename)
    ...
    print(f"\nVideo {video_filename} processing complete!")
...
print(f"Processed {len(video_files)} videos total")
print(f"{'='*60}")
